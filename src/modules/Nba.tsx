...
import { 
  RefreshCw, Search, Copy, Check, ChevronDown, Trash2, Plus, Pencil, Zap, Dumbbell 
} from 'lucide-react';
import { PYTHON_BACKEND_URL, getRandomKey, SPORTS_CONFIG } from '../config';

export default function Nba() {
  const [matches, setMatches] = useState([]);
  const [status, setStatus] = useState("SISTEMA NBA LISTO");
  ...
  const escanear = async () => {
    ...
    setStatus("BUSCANDO JUEGOS NBA...");
    try {
      const apiKey = getRandomKey();
      // En NBA pedimos spreads (Hándicap) y totals (Over/Under)
...
- Diferencia: ${data.math.elo_diff_adjusted} puntos.
- **PROYECCIÓN:** ${data.math.favorito} gana por **${Math.abs(data.math.expected_margin)} puntos**.
### 4. 📉 LÍNEAS DE MERCADO:
${linesFormatted}
---
### 🕵️‍♂️ MISIÓN TÁCTICA (BUSCAR EN GOOGLE):
1. **MATEMÁTICA:** Cruza la ventaja de ${data.math.expected_margin} puntos con las líneas (Spreads).
...
3. **REPORTE DE LESIONES:**
   - Busca "NBA Injury Report Today". ¿Juegan las estrellas (LeBron, Curry, Giannis)?
   - Si falta la estrella, descarta la matemática y ajusta a la baja.
                ...
                {!generatedPrompts[m.id] ? (
                    <div className="space-y-3">
                        <div className="flex gap-2">
                            <input type="number" placeholder="Power Rating Local" onChange={(e)=>handleRatingChange(m.id,'home',e.target.value)} className="w-full bg-black border border-white/20 p-2 text-center text-white text-xs rounded outline-none focus:border-orange-500"/>
                            <input type="number" placeholder="Power Rating Visita" onChange={(e)=>handleRatingChange(m.id,'away',e.target.value)} className="w-full bg-black border border-white/20 p-2 text-center text-white text-xs rounded outline-none focus:border-orange-500"/>
                        </div>
                        ...
                        <button onClick={()=>generarPrompt(m)} className="w-full border border-dashed border-orange-500/30 text-orange-400 py-2 rounded text-xs font-bold hover:bg-orange-900/10">PROCESAR NBA</button>
                    </div>
                ) : (
